...
class ChunkedGenerator:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
        ...
        if forces is not None:
            self.batch_grf = np.empty((batch_size, chunk_length, forces[0].shape[-1]))
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    ...
                    #GRF forces (if applicable) - center predict, just like 3D poses
                    if self.forces is not None:
                        seq_grf = self.forces[seq_i]
                        low_grf = max(start_3d, 0)
                        high_grf = min(end_3d, seq_grf.shape[0])
                        pad_left_grf = low_3d - start_3d
                        pad_right_grf = end_3d - high_3d
                        if pad_left_grf != 0 or pad_right_grf != 0:
                            self.batch_grf[i] = np.pad(seq_grf[low_grf:high_grf], ((pad_left_grf, pad_right_grf), (0, 0), (0, 0)), 'edge')
                        else:
                            self.batch_grf[i] = seq_grf[low_grf:high_grf]

                        if flip:
                            #Swap left and right force plate positions
                            grf_copy = np.copy(self.batch_grf[i,:])
                            self.batch_grf[i,:,:3],self.batch_grf[i,:,3:] = grf_copy[:,3:],grf_copy[:,:3]
                ...
                if self.poses_3d is None and self.cameras is None:
                    ...
                    yield self.batch_cam[:len(chunks)], self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], self.batch_grf[:len(chunks)]
                else:
...
class UnchunkedGenerator:
    ...
    def __init__(self, cameras, poses_3d, poses_2d, pad=0, causal_shift=0,
                 augment=False, kps_left=None, kps_right=None, joints_left=None, joints_right=None, forces=None):
    ...
    def next_epoch(self):
        for seq_cam, seq_3d, seq_2d, seq_force in zip_longest(self.cameras, self.poses_3d, self.poses_2d, self.forces):
            batch_cam = None if seq_cam is None else np.expand_dims(seq_cam, axis=0)
            batch_3d = None if seq_3d is None else np.expand_dims(seq_3d, axis=0)
            batch_grf = None if seq_force is None else np.expand_dims(seq_force, axis=1)
            ...
            if self.augment:
                ...
                if batch_force is not None:
                    batch_grf_flipped = np.copy(batch_grf)
                    batch_grf_flipped[:,:,:3], batch_grf_flipped[:,:,3:] = batch_grf[:,:,3:], batch_grf[:,:,:3]
                    batch_grf = np.concatenate((batch_grf, batch_grf_flipped), axis=0)

                batch_2d = np.concatenate((batch_2d, batch_2d), axis=0)
                batch_2d[1, :, :, 0] *= -1
                batch_2d[1, :, self.kps_left + self.kps_right] = batch_2d[1, :, self.kps_right + self.kps_left]

            yield batch_cam, batch_3d, batch_2d, batch_grf
