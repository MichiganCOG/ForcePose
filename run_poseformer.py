...
dataset_path = args.dataset_root+ '/data_3d_' + args.dataset + '.npz'
...
if args.dataset.startswith('parkour'):
    neighbor_links = [[1,5], [1,2], [2,3], [3,4],
		      [5,6], [6,7], [7,8],
		      [13,14], [14,15], [15,0],
		      [9,10], [10,11], [11,12],
		      [13,5], [9,1], [9,13]]
else:
    neighbor_links = [[16,14],[14,12],[17, 15],[15,13],
                     [12,13],[6,12], [7,13],[6,7],
    ...
    for action in dataset[subject].keys():
        ...
        if 'positions' in anim:
            ...
            for cam in anim['cameras']:
                if 'positions_triangulated' in anim:
                    #anim['positions_triangulated'][:,:,[1,2]] = anim['positions_triangulated'][:,:,[2,1]] #Swap Y-Z axes
                ...
                #lines1 = ax1.plot([], [], [], color='black')[0]
                #lines2 = ax2.plot([], [], [], color='red')[0]

                xdata = anim['positions'][0,:,0]
                ydata = anim['positions'][0,:,1]
                zdata = anim['positions'][0,:,2]

                for i,j in neighbor_links:
                    ax1.plot([xdata[i-1],xdata[j-1]], [ydata[i-1],ydata[j-1]], [zdata[i-1],zdata[j-1]], 'ro-')
                    ax2.plot([pos_3d[0,i-1,0],pos_3d[0,j-1,0]], [pos_3d[0,i-1,1],pos_3d[0,j-1,1]], [pos_3d[0,i-1,2], pos_3d[0,j-1,2]], 'bo-')
...
model_pos_train = PoseTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=32, depth=4,
        num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0.1, pred_force=args.pred_force)

model_pos = PoseTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=32, depth=4,
        num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0, pred_force=args.pred_force)
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False):
    ...
    with torch.no_grad():
        ...
        # else:
            # model_traj.eval()
        N = 0
        for _, batch, batch_2d, batch_grf in test_generator.next_epoch():
            inputs_2d = torch.from_numpy(batch_2d.astype('float32'))
            inputs_3d = torch.from_numpy(batch.astype('float32'))
            ...
            if return_predictions:
                predictions.append(predicted_3d_pos.squeeze(0).cpu().numpy())
                continue
    ...
    if return_predictions:
        return np.stack(predictions) * 1000 #Stack 3D position estimates from multiple views and scale back up 
...
if args.render:
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        for action_key in actions.keys():
            ...
            if args.save_pred: #Unique to Force Pose dataset. Saving 3D coordinate predictions
                if len(actions[action_key]) > 1: #Separate each run/action by subject
                    num_subj = len(actions[action_key])
                    for aidx in range(num_subj):
                        start = int(aidx * len(poses_act)/num_subj)
                        stop  = int((aidx+1) * len(poses_act)/num_subj)
                        gen = UnchunkedGenerator(None, poses_act[start:stop], poses_2d_act[start:stop],
                             pad=pad, causal_shift=causal_shift, augment=args.test_time_augmentation,
                             kps_left=kps_left, kps_right=kps_right, joints_left=joints_left,
                             joints_right=joints_right)

                        prediction = evaluate(gen, action_key, return_predictions=True)
                        save_dir = os.path.join('./saved_coordinates',args.exp_name)
                        os.makedirs(save_dir, exist_ok=True)
                        np.save(os.path.join(save_dir, actions[action_key][aidx][0]+'_'+action_key), prediction)
                else:
                    prediction = evaluate(gen, action_key, return_predictions=True)
                    save_dir = os.path.join('./saved_coordinates',args.exp_name)
                    os.makedirs(save_dir, exist_ok=True)
                    np.save(os.path.join(save_dir, actions[action_key][0][0]+'_'+action_key), prediction)
            else:
